...
// Interface para os dados persistidos em nuvem por CNPJ
...
// Higienizar CNPJ
function sanitizeCnpj(rawCnpj: any): string {
  if (!rawCnpj) return 'default';
  const clean = String(rawCnpj).replace(/\D/g, '').trim();
  return clean || 'default';
}

// Extrair CNPJ da requisição (Header, Query, Body ou Rota)
function extractCnpjFromReq(req: express.Request): string {
  const fromHeader =
  ...
  const fromParams = req.params && (req.params as any).cnpj;
...
function getTenantDbFilePath(cnpj: string): string {
  const clean = sanitizeCnpj(cnpj);
  return path.join(TENANTS_DIR, `cloud_db_${clean}.json`);
}

// Inicializar ou carregar banco dinâmico do CNPJ (criando ZERADO se novo)
function loadDatabaseForCnpj(cnpj: string, companyMeta?: { razaoSocial?: string; nomeFantasia?: string }): CloudDatabase {
  ...
  // Se não existir, cria dinamicamente uma base 100% ZERADA para o CNPJ
  ...
  console.log(`[MultiTenant Cloud] ✅ Novo banco de dados ZERADO criado para o CNPJ: ${cleanCnpj}`);
  return zeroDb;
}

function saveDatabaseForCnpj(cnpj: string, db: CloudDatabase) {
  const cleanCnpj = sanitizeCnpj(cnpj);
  try {
    ...
    console.error(`[MultiTenant Cloud] Erro ao gravar banco do CNPJ ${cleanCnpj}:`, error);
  }
}

// Helper para mesclar listas por ID
function mergeById(existingList: any[] = [], incomingList: any[] = []): any[] {
...
async function startServer() {
  ...
  // Middleware multi-tenant: anota CNPJ ativo no cabeçalho de resposta
  ...
  // ==================== ROTAS DE API MULTI-TENANT POR CNPJ ====================
  ...
  app.get('/api/health', (req, res) => {
    ...
    res.json({
      status: 'online',
      message: 'JM Sistemas Cloud Sync Multi-Tenant API Online',
      activeCnpj: cnpj,
  ...
  // 3. Criar explicitamente um banco zerado para um novo CNPJ
  app.post('/api/tenant/create', (req, res) => {
    try {
      const { cnpj, razaoSocial, nomeFantasia } = req.body;
      if (!cnpj) {
        return res.status(400).json({ success: false, error: 'CNPJ é obrigatório para inicializar a base.' });
      }
      ...
      res.json({
        ...
        isNew: !exists,
        message: exists
          ? `Base para o CNPJ ${cleanCnpj} já existe e está pronta para uso.`
  ...
  // 5. POST /api/sync/push - Enviar e unificar pedidos, clientes e financeiro para o CNPJ
  app.post(['/api/sync/push', '/api/tenant/:cnpj/sync/push'], (req, res) => {
    try {
      ...
      console.error('[MultiTenant Push Error]:', err);
      res.status(500).json({ success: false, error: err.message });
    }
  });

  // 6. GET /api/orders - Listar todos os pedidos daquele CNPJ
  ...
  // 8. POST /api/reset-db - Resetar banco do CNPJ para testes
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[JM Sistemas Server] Servidor e API em Nuvem Multi-Tenant ativos na porta ${PORT}`);
  });
}
