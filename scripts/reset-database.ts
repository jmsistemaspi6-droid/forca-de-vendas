/**
 * ============================================================================
 * SCRIPT DE RESET E CRIAÇÃO DE TABELAS VAZIAS (TypeScript / Node.js)
 * ============================================================================
 * Executa o reset completo do banco de dados, recriando as tabelas principais:
 *  - PRODUTOS
 *  - CLIENTES
 *  - CONTAS_A_RECEBER
 * 
...
// Definição dos Schemas SQL para Bancos Relacionais (PostgreSQL / SQLite / MySQL)
...
export async function resetDatabase() {
  ...
  // 1. Gera o arquivo SQL de DDL limpo para o banco de dados
  const sqlOutputDir = path.resolve(process.cwd(), 'database');
  if (!fs.existsSync(sqlOutputDir)) {
    fs.mkdirSync(sqlOutputDir, { recursive: true });
  }

  const sqlFilePath = path.join(sqlOutputDir, 'schema_clean.sql');
  fs.writeFileSync(sqlFilePath, SQL_SCHEMAS.trim(), 'utf-8');
  console.log(`✅ Schema SQL DDL exportado com sucesso: ${sqlFilePath}`);

  // 2. Criação dos arquivos JSON de base vazia inicial para o storage do app
  const emptySeedDir = path.resolve(process.cwd(), 'src/data/empty_database');
  if (!fs.existsSync(emptySeedDir)) {
    fs.mkdirSync(emptySeedDir, { recursive: true });
  }

  const emptyProducts: any[] = [];
  const emptyClients: any[] = [];
  const emptyReceivables: any[] = [];

  fs.writeFileSync(
    path.join(emptySeedDir, 'produtos.json'),
    JSON.stringify(emptyProducts, null, 2),
    'utf-8'
  );
  fs.writeFileSync(
    path.join(emptySeedDir, 'clientes.json'),
    JSON.stringify(emptyClients, null, 2),
    'utf-8'
  );
  fs.writeFileSync(
    path.join(emptySeedDir, 'contas_a_receber.json'),
    JSON.stringify(emptyReceivables, null, 2),
    'utf-8'
  );

  console.log(`✅ Tabelas vazias preparadas em: ${emptySeedDir}`);
  console.log('   - PRODUTOS: 0 registros (estrutura pronta)');
  console.log('   - CLIENTES: 0 registros (estrutura pronta)');
  console.log('   - CONTAS_A_RECEBER: 0 registros (estrutura pronta)');
  console.log('===============================================================');
  console.log(' 🎉 BANCO DE DADOS 100% LIMPO E PRONTO PARA NOVOS CLIENTES!');
  console.log('===============================================================');
}
